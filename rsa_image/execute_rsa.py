...
from imgstegan import Encode, Decode
import math
import sys
...
# encrypts a plaintext message using the current key
    ...
    plaintext = (plaintext.lower())#converts text to lowercase
    numC = []
    for i in range(len(plaintext)):
        for j in range(len(letter)):
            if(plaintext[i]==letter[j]):
                numC.append(number[j])#transform to numerical format
    cipher(numC,e)#encrypting message to ciphertext using public key e
    print("Ciphertext:", X)
